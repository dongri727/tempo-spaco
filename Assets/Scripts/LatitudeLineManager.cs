...
public class LatitudeLineManager : MonoBehaviour
{
    ...
    void Start()
    {
        northpole = Instantiate(selectedPrefab, new Vector3(0, 100, 0), Quaternion.identity);
        southpole = Instantiate(selectedPrefab, new Vector3(0, -100, 0), Quaternion.identity);
    ...
    void LoadAndInstantiateObjects(string filename, List<GameObject> objectList)
    {
        ...
        for (int i = 1; i < lines.Length; i++)
        {
            ...
            float coord_x = float.Parse(parts[0]) * 100;
            float coord_y = float.Parse(parts[1]) * 100;
            float coord_z = float.Parse(parts[2]) * 100;
